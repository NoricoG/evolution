...
export class Strategy {
    ...
    constructor(diet: Diet, genes: { [key: string]: Gene }) {
        this.chromosome = new Chromosome(Strategy.groups, genes);

        if (diet != Diet.HERBIVORE && diet != Diet.OMNIVORE) {
            this.chromosome.genes["GatherAction"].setToNull();
        }
        if (diet != Diet.OMNIVORE && diet != Diet.CARNIVORE) {
    ...
    static random(diet: Diet): Strategy {
        const chromosome = Chromosome.random(Strategy.groups);
    ...
    static similar(strategyA: Strategy, strategyB: Strategy): boolean {
    ...
    toColorOld(): string {
        ...
        for (const [action, weight] of Object.entries(this.chromosome.genes)) {
            ...
            r += rc * weight.value!; g += gc * weight.value!; b += bc * weight.value!;
            total += weight.value!;
        }
    ...
    decide(actions: Action[], individual: Individual): Action {
